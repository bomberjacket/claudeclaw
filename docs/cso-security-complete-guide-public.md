...
**Project:** Multi-agent Telegram bot framework
...
This framework has a dedicated security system consisting of two parts:
...
The CSO (Chief Security Officer) agent has a single skill: `security`. If it identifies a need for additional capabilities, it escalates through the orchestrator agent, who relays the request to the owner for approval. CSO never self-assigns skills.
...
Owner (Telegram)
  ...
  |           |
  |           v
  |     Orchestrator relays request to Owner
  |           |
  |           v
  |     Owner approves/denies
  |
  +-- @YourOrchestrator_bot (Orchestrator agent)
  +-- @YourCMO_bot (CMO agent -- marketing)
  +-- @YourPA_bot  (PA -- personal assistant)
```

### Why This Architecture is Maximum-Risk

This system satisfies Simon Willison's **Lethal Trifecta** -- all three present simultaneously:
...
- [ ] All API keys treated as critical credentials
...
- [ ] Bot tokens not exposed in logs
...
- [ ] Budget hard-limit (95%) functioning -- all calls downgrade to cheaper model when triggered
...
|----------|---------|---------|
| `PROJECT_DIR` | `<your-project-root>` | Project root (or set env var) |
...
| FS | File ACLs | `icacls` on .env, store/, database file |
...
pm2 stop <your-app-name> 2>/dev/null

# Kill any running node processes related to your app
taskkill /F /IM node.exe /FI "WINDOWTITLE eq <your-app>" 2>nul
...
```bash
# Snapshot logs
pm2 logs <your-app-name> --nostream --lines 1000 > "$HOME/incident-$(date +%Y%m%d-%H%M%S).log"

# Snapshot database
cp "<project-root>/store/<your-db>.db" "$HOME/incident-db-$(date +%Y%m%d-%H%M%S).sqlite"
...
sqlite3 "<project-root>/store/<your-db>.db" "SELECT * FROM token_budget ORDER BY created_at DESC LIMIT 50;" > "$HOME/incident-budget-$(date +%Y%m%d-%H%M%S).txt"
```

**Step 3. Assess**
```bash
# Check conversation log for suspicious commands
sqlite3 "<project-root>/store/<your-db>.db" "
...
powershell -Command "Get-ChildItem -Path '<project-root>' -Recurse -File | Where-Object { $_.LastWriteTime -gt (Get-Date).AddHours(-24) } | Select-Object -First 50 FullName, LastWriteTime"

# Check for scheduled tasks referencing your app
schtasks /query /fo LIST | findstr -i <your-app>
```
...
pm2 start <your-app-name> 2>/dev/null || npm start
```
...
|-----------|----------|-----|
| Telegram bot tokens | On compromise or annually | BotFather > Revoke Token |
| Anthropic API key | On compromise or quarterly | console.anthropic.com > API Keys |
| Google API key | On compromise or quarterly | console.cloud.google.com > Credentials |
| Slack token | On compromise or annually | Slack App > OAuth & Permissions > Reinstall |
...
| Cheap-model-only spend when both keys available | Budget hard-limit triggered unexpectedly | `token_budget` table |
...
This framework uses a Token Optimization Framework to route API calls across providers and track spend.
...
- **95%** -- Hard-limit: ALL calls forced to cheapest available model
...
sqlite3 store/<your-db>.db "
  ...
  GROUP BY date_key HAVING total > (TOKEN_DAILY_BUDGET_USD * 2) ORDER BY date_key DESC;"
```
...
telegram_bot_token_env: <YOUR_CSO_BOT_TOKEN_ENV>
...
- **Escalation chain**: CSO -> hive_mind capability_request -> Orchestrator -> Owner
- **Daily scan workflow**: 8-step sequence at scheduled time
...
A daily scan runs automatically at a configured time. CSO follows the full Daily Scan Workflow and reports results via Telegram.

```bash
cd <project-root>
node dist/schedule-cli.js list --agent cso          # list tasks
node dist/schedule-cli.js pause <task-id>            # pause daily scan
node dist/schedule-cli.js resume <task-id>           # resume daily scan
node dist/schedule-cli.js delete <task-id>           # delete daily scan
```

### On-Demand (Telegram)

Message your CSO bot directly:
...
CSO detects issue
  |
  +-- P3/P2 finding --> Logs to security_findings table, reports to Owner
  |
  +-- P1 critical vuln --> Logs finding + alerts Owner immediately
  |
  +-- P0 active breach --> Stops bot, preserves evidence, alerts Owner
  |
  +-- Needs new capability --> Logs capability_request to hive_mind
                                Asks Orchestrator to relay to Owner
```
...
module.exports = {
  apps: [
    { name: '<app>-pa',  script: 'dist/index.js' },
    { name: '<app>-cos', script: 'dist/index.js', args: '--agent cos' },
    { name: '<app>-cso', script: 'dist/index.js', args: '--agent cso' },
    { name: '<app>-cmo', script: 'dist/index.js', args: '--agent cmo' },
  ],
};
```
...
| `pm2 logs <app>-cso` | Tail CSO logs |
| `pm2 logs <app>-cso --nostream --lines 50` | Last 50 lines |
...
4. Start: `pm2 start dist/index.js --name <app>-<name> -- --agent <name>`
5. Save: `pm2 save`

### Log Locations

PM2 logs are at `<your-home-dir>/.pm2/logs/`:
- `<app>-cso-out.log` -- stdout
- `<app>-cso-error.log` -- stderr
...
| Kill process by name | `pkill -f <app>` | `taskkill /F /IM node.exe /FI "WINDOWTITLE eq <app>"` |
...
| Check scheduled tasks | `crontab -l` | `schtasks /query /fo LIST \| findstr -i <app>` |
...
This runs in Git Bash on Windows. This means:
...
| Security findings DB | `store/<your-db>.db` (table: `security_findings`) | Active |
| Token budget DB | `store/<your-db>.db` (table: `token_budget`) | Active |
